-- =====================================================
-- MIGRAÇÃO CORRIGIDA - CRIA COLUNAS PRIMEIRO, DEPOIS CONSTRAINTS
-- =====================================================
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE table_name = 'simulations' 
        AND constraint_name = 'simulations_status_check'
    ) THEN
        ALTER TABLE simulations ADD CONSTRAINT simulations_status_check 
        CHECK (status IN ('created', 'running', 'completed', 'cancelled'));
        RAISE NOTICE 'Constraint status adicionada';
    END IF;
END $$;
...
SELECT 'Migração corrigida concluída com sucesso!' as status;
