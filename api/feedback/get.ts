...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { simulationId, stationNumber } = req.query;
    
    if (!simulationId) {
      return res.status(400).json({ ok: false, error: 'missing_simulation_id' });
    }
    
    console.log(`📋 Buscando feedback para simulação ${simulationId}`);
    
    const supabase = getSupabaseAdmin();
    
    if (stationNumber) {
      // Buscar feedback de uma estação específica
      const { data: feedback, error } = await supabase
        .from('feedback')
        .select('*')
        .eq('simulation_id', simulationId)
        .eq('station_number', parseInt(stationNumber as string))
        .single();
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          return res.status(404).json({ 
            ...
            message: 'Feedback ainda não foi gerado para esta estação'
          });
        }
        throw error;
      }
      ...
      // Buscar todos os feedbacks da simulação
      ...
      if (!feedbacks || feedbacks.length === 0) {
        return res.status(404).json({ 
          ...
          message: 'Nenhum feedback foi gerado para esta simulação ainda'
        });
      }
      ...
      const consolidatedFeedback = {
        ...
        summary: {
          ...
          overallAssessment: summary?.average_percentage >= 70 
    ...
    return res.status(500).json({
      ok: false,
      error: 'feedback_retrieval_failed',
